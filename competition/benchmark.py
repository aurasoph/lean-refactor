...
benchmark will be released on Oct 1, 2026.
...
# Local addition (not in the Space): problems we promoted ourselves from
# mining/ (compile-verified upstream proofs, same row schema). Kept in a
# separate file so the official warm-up set stays byte-identical.
_EXTRA_PATH = Path(__file__).resolve().parent / "benchmark_data_extra.jsonl"
...
def _load() -> dict[str, dict]:
    ...
    lines: list[str] = []
    for path in (_DATA_PATH, _EXTRA_PATH):
        if path.exists():
