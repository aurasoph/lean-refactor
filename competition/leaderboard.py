...
class Leaderboard:
    ...
    @staticmethod
    def _compute_user_summary(results: dict) -> dict:
        """Aggregate metrics over all benchmark theorems.

        Returns a dict with avg reduction %s, compiled count, and survival rate.
        All averages are over the full benchmark — missing/non-compiled entries
        contribute 0 to the numerator.  Survival rate counts compat passes only
        over compiled theorems.
        """
        ...
        for name in names:
            ...
            if r.get("compiled"):
                total_len_pct += float(r.get("length_reduction_pct", 0.0))
                total_len_abs += float(r.get("length_abs_reduction", 0.0))
                total_hb_pct += float(r.get("heartbeat_reduction_pct", 0.0))
        ...
        n = len(names)
        survival = (
            round(compat_passed / compat_checks * 100.0, 1)
            if compat_checks > 0
            else 0.0
        )
        return {
    ...
    def submit(
        ...
        # Combined = mean of the two reductions and the zero-shot transfer rate.
        combined = (
            summary["avg_len_pct"]
            + summary["avg_hb_pct"]
